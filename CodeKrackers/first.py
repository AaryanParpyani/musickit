    ...
    for i in range(beats):
        for j in range(instruments):
            if clicked[j][i]==-1:
                ...
                color=green
            rect = pygame.draw.rect(screen, color, [i * ((WIDTH - 200) // beats) + 210, (j * 100)+5, ((WIDTH - 200) // beats)-10, ((HEIGHT - 200)//instruments)-10], 0,3 )
            pygame.draw.rect(screen, gold, [i * ((WIDTH - 200) // beats) + 205, (j * 100), ((WIDTH - 225) // beats), ((HEIGHT - 200)//instruments)], 5, 5)

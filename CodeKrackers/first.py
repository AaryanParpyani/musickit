...
load_menu = False
saved_beats = []
file = open('saved_beats.txt', 'r') #read all the information aboout beats, store data from file
for line in file:
...
def draw_save_menu(beat_name, typing):
    ...
    menu_text = label_font.render('Enter preferred name for your work: ', True, white)
    ...
    exit_btn = pygame.draw.rect(screen, gray, [WIDTH - 175, HEIGHT - 100, 100, 50], 0, 5)
...
def draw_load_menu(index):
    ...
    loading_btn = pygame.draw.rect(screen, gray, [WIDTH // 2 - 160, HEIGHT * 0.87, 400, 100], 0,5)
    loading_txt = label_font.render('Load Beat', True, white)
    screen.blit(loading_txt, (WIDTH //2 - 30, HEIGHT * 0.87 + 30))

    delete_btn = pygame.draw.rect(screen, gray, [(WIDTH // 2) - 510, HEIGHT * 0.87, 200, 100], 0, 5)
    ...
    exit_btn = pygame.draw.rect(screen, gray, [WIDTH - 210, HEIGHT * 0.87, -100, 100], 0, 5)
    exit_text = label_font.render("close", True, white)
...
#we will check at bottom if we have a menu active or not
    if save_menu:
        exit_button, saving_button, entry_rectangle, beat_name = draw_save_menu(beat_name, typing) #saving_btn to check whether we are saving the beat or not

    if load_menu:
        exit_button, loading_button, delete_button, entry_rectangle, loaded_info = draw_load_menu(index)
    ...
    for event in pygame.event.get(): #any event taking place
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            ...
            if exit_button.collidepoint(event.pos): #we just want a way to open the menu and exit the menu
                save_menu = False
                load_menu = False 
                playing = True
                typing = False  
            ...
            if entry_rectangle.collidepoint(event.pos):  #click on entry rectangle should change state of typing
                if save_menu:
                    if typing:
                        typing = False
                    else:
        ...
        if event.type == pygame.KEYDOWN: #type of event(keydwown) and checking if
            if event.key == pygame.K_BACKSPACE and len(beat_name) > 0 and typing:
                beat_name = beat_name[:-1] #(nothing before colon - start from beginning) and -1 means go to char right before end of string
